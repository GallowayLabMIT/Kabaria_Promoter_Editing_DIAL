...
csvwrite('kcat_noCre_ZF37.csv', bootstrap_params_k_cat_noCre);
csvwrite('kcat_Cre_ZF37.csv', bootstrap_params_k_cat_Cre);
csvwrite('KM_noCre_ZF37.csv', bootstrap_params_K_M_noCre);
csvwrite('KM_Cre_ZF37.csv', bootstrap_params_K_M_Cre);
...
xlabel('mCherry-A Normalized by gmean of ZF37 0.125x');
ylabel('mGL-A  Normalized by gmean of ZF37 0.125x');
title('Bootstrapped Curves for ZF37');
...
%% exporting the 95% CI to plot using seaborn

% Calculate the 95% CI data points for x_zf_noCre
ci_data_noCre = [x_values', lower_percentiles_noCre, upper_percentiles_noCre];

ci_data_noCre_real = real(ci_data_noCre);

% Save the data points for x_zf_noCre into a CSV file
csvwrite('ci_data_noCre_ZF37.csv', ci_data_noCre_real);

% Calculate the 95% CI data points for x_zf_Cre
ci_data_Cre = [x_values', lower_percentiles_Cre, upper_percentiles_Cre];

ci_data_Cre_real=real(ci_data_Cre);

% Save the data points for x_zf_Cre into a CSV file
csvwrite('ci_data_Cre_ZF37.csv', ci_data_Cre_real);
